...
for sel_segid in ['A','B','C','D'] : #change to your chain here
    datas_seg = []
    datas = np.zeros([1,400]) #put the TOTAL number of residue from chain A and a total number of residue of chain E. This can be over - is just making an array
    contacts = generate.build_contacts(ligand_selection= "resname CHOL", protein_selection='segid ' + str(sel_segid), frameskip=1,cutoff=6,KDTree=True) #this will be other chain - I put E here as my protein is a tetramer. But change to whatever. Cutoff = 4. Change frameskip if you want. I recommend you read less than 1000 frames.
    ...
    datas_seg = np.array(datas_seg) #Save data as matrix
    datas_av = np.average(datas_seg, axis = 0)
    np.savetxt(sys.argv[3] + str(sel_segid) + ".dat",datas_av) 
    df = pd.DataFrame(datas_av).unstack() #Show as list
    df.index.names = ['ref', 'resid']
    df = df.reset_index()
    df.to_csv(sys.argv[3] + str(sel_segid) + ".csv")
